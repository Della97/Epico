...
use crate::config::{MorphAction, NodeSpec, PipelineSpec, StageSource, StageSpec};
...
fn write_runtime_yaml(
    ...
    // Pre-declared physical alternatives (M2). The agent widens the affected
    // replica-index spaces at boot for each pair, which is what lets a fused
    // replica bind valid consumer/producer indices with no runtime mesh swap.
    // Emitted only when non-empty so unchanged pipelines produce byte-identical
    // runtime.yaml.
    let mut fusible_block = String::new();
    if !spec.fusible.is_empty() {
        fusible_block.push_str("fusible:\n");
        for (a, b) in &spec.fusible {
            fusible_block.push_str(&format!("  - [{a}, {b}]\n"));
        }
        fusible_block.push('\n');
    }

    // Deterministic morph schedule. Morphs at KNOWN instants are what make
    // break-even measurable; a controller deciding for itself is a later
    // producer of the same requests.
    let mut morphs_block = String::new();
    if !spec.morphs.is_empty() {
        morphs_block.push_str("morphs:\n");
        for m in &spec.morphs {
            let action = match &m.action {
                MorphAction::Fuse(a, b)   => format!("fuse: [{a}, {b}]"),
                MorphAction::Split(name)  => format!("split: {name}"),
                MorphAction::Identity(nm) => format!("identity: {nm}"),
            };
            morphs_block.push_str(&format!("  - {{ at_s: {}, {action} }}\n", m.at_s));
        }
        morphs_block.push('\n');
    }

    // The cost-model controller — the alternative producer of those same
    // requests. Emitted only when a pipeline declares one, and only the knobs
    // it actually set: every omitted field falls back to the agent's
    // measurement-derived default, and writing those out here would freeze a
    // copy of them into every generated runtime.yaml.
    let mut controller_block = String::new();
    if let Some(c) = spec.controller.as_ref() {
        controller_block.push_str("controller:\n");
        controller_block.push_str(&format!("  enabled: {}\n", c.enabled));
        for (name, v) in [
            ("period_s",              c.period_s),
            ("alpha",                 c.alpha),
            ("cooldown_fuse_s",       c.cooldown_fuse_s),
            ("cooldown_split_s",      c.cooldown_split_s),
            ("significance_floor_us", c.significance_floor_us),
            ("min_rate_eps",          c.min_rate_eps),
        ] {
            if let Some(v) = v {
                controller_block.push_str(&format!("  {name}: {v}\n"));
            }
        }
        if let Some(w) = c.window_s {
            controller_block.push_str(&format!("  window_s: {w}\n"));
        }
        controller_block.push('\n');
    }
