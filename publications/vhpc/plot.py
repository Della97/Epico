...
MODE_ORDER = ["AOT", "JIT"]
STAGE_ORDER = ["warmup", "normalize", "detect", "finalize"]
...
def plot_first_spawn(df_cold: pd.DataFrame, out_dir: Path):
    ...
    for ax, stage in zip(axes, STAGE_ORDER):
        ...
        ax.set_xlabel("")
        # Tasteful y-axis padding
        ymax = sub["cold_start_ms"].max() * 1.15
        ax.set_ylim(0, ymax)
...
def plot_cdf(df_cold: pd.DataFrame, out_dir: Path):
    ...
    # Each stage gets a distinct line style. Solid for the most-loaded stage.
    stage_styles = {
        "normalize": "solid",
        "detect":    (0, (4, 1.5)),       # dashed
        "finalize":  (0, (1, 1.5)),       # dotted
    }
...
def plot_decomposition(df_cold: pd.DataFrame, out_dir: Path):
    ...
    For JIT first-spawn, cold_start_ms includes:
      compile + instantiate_pre + residual (thread/socket setup, page faults)

    For AOT first-spawn, cold_start_ms is essentially residual (compile and
    instantiate_pre happen before the cold-start window starts).

    Horizontal layout reads better than vertical for stacked components,
    and we show a separate "AOT compile (outside window)" striped bar
    for honesty — it's not zero work, it's just paid earlier.
    """
    ...
    # Color tints for each component within a mode's main color
    def tint(base_hex: str, mix: float) -> str:
        """Mix base_hex toward white by `mix` fraction (0..1)."""
    ...
    for stage in STAGE_ORDER:
        for mode in MODE_ORDER:
            ...
            if mode == "JIT":
                ...
                resid = max(row["cold_start_ms"] - comp - inst, 0.0)
                # Stack: residual | instantiate_pre | compile
                ax.barh(y, resid, height=bar_height,
                        color=color, alpha=0.95, edgecolor="white", linewidth=0.5)
                ax.barh(y, inst, height=bar_height, left=resid,
                        color=tint(color, 0.45), edgecolor="white", linewidth=0.5)
                ax.barh(y, comp, height=bar_height, left=resid + inst,
                        color=tint(color, 0.7), edgecolor="white", linewidth=0.5)
                total = resid + inst + comp
                ax.text(total + 0.3, y, f"{total:.2f} ms",
                        va="center", ha="left", fontsize=9,
                        color=COLOR_ANNOT, fontweight="semibold")
            else:  # AOT
                resid = row["cold_start_ms"]
                ax.barh(y, resid, height=bar_height,
                        color=color, alpha=0.95, edgecolor="white", linewidth=0.5)
                # Show AOT compile as a ghosted bar to the left of zero
                # to make clear it exists but is paid outside the window
                ...
                ax.text(resid + 0.3, y, f"{resid:.2f} ms",
                        va="center", ha="left", fontsize=9,
                        color=COLOR_ANNOT, fontweight="semibold")
    ...
    # Legend
    legend_handles = [
        mpatches.Patch(facecolor=COLOR_NEUTRAL, alpha=0.95, label="residual (thread setup, instantiate, page faults)"),
        mpatches.Patch(facecolor=tint(COLOR_NEUTRAL, 0.45), label="instantiate_pre (JIT only, inside window)"),
        mpatches.Patch(facecolor=tint(COLOR_NEUTRAL, 0.7), label="compile (JIT: inside window)"),
...
def write_summary_stats(df_cold: pd.DataFrame, out_dir: Path):
    ...
    pivot = (summary[summary["spawn_kind"] == "first"]
             .pivot(index="stage", columns="mode", values="median_ms")
             .reindex(STAGE_ORDER))
    pivot["ratio (JIT/AOT)"] = pivot["JIT"] / pivot["AOT"]
...
def main():
    ...
    if args.from_csv:
        ...
        df_cold.to_csv(out_dir / "cold_starts_long.csv",
                       index=False, float_format="%.4f")
        print(f"  wrote {out_dir / 'cold_starts_long.csv'}")
